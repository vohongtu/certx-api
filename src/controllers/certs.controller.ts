...
export async function issue(req: any, res: any) {
  ...
  const { holderName, degree, issuedDate, issuerName } = req.body
  const docHash = sha256Hex(file)

  // Metadata đơn giản lưu DB (dễ làm). Có thể đổi sang IPFS nếu muốn.
  const metadata = { holderName, degree, issuedDate, issuerName, docHash }
  // const metadataUri = await uploadJSON(config.IPFS_TOKEN, metadata)
  const metadataUri = '' // để trống ở MVP
...
export async function verify(req: any, res: any) {
  ...
  try {
    ...
    const status = Number(on.status) === 1 ? 'VALID' : Number(on.status) === 2 ? 'REVOKED' : 'NOT_FOUND'
